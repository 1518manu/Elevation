...
export default function ProcessStepper() {
  ...
  return (
    <div className="relative">
      <div className="hidden md:absolute md:left-0 md:right-0 md:top-8 md:block md:h-0.5 md:bg-accent/30" />
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5 lg:grid-cols-10">
        {steps.map((step, i) => {
          ...
          return (
            ...
            >
              <div className="relative z-10 mb-3 flex h-16 w-16 items-center justify-center rounded-full border-2 border-accent bg-white shadow-card">
                <Icon className="h-6 w-6 text-primary" />
                <span className="absolute -right-1 -top-1 flex h-6 w-6 items-center justify-center rounded-full bg-accent text-xs font-bold text-primary">
                  {step.step_number}
                </span>
              </div>
