      ...
      <section className="py-16">
        <div className="mx-auto max-w-4xl px-4">
          ...
          {service.key_features?.length > 0 && (
            <div>
              <h2 className="mb-4 font-heading text-2xl font-semibold text-primary">Key Features</h2>
              <ul className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {service.key_features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2"><Check className="h-5 w-5 text-accent" />{f}</li>
                ))}
              </ul>
            </div>
          )}
          <Button onClick={openModal} className="mt-8 bg-accent text-primary hover:bg-accent-dark">Get Free Quote</Button>
        </div>
      </section>
      <CTABanner />
    </>
